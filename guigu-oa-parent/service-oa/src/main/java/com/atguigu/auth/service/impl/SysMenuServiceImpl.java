...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public void removeMenuById(Long id) {
        ...
        //判断count是否大于0：大于就表示有下一层不能删除
        if (count > 0) {
            throw new GuiguException(201, "当前菜单下有子菜单，不能删除");
        }
    ...
    public List<SysMenu> findMenuByRoleId(Long roleId) {
        ...
        //3.1拿着菜单id 和所有菜单集合里面的id进行比较，如果相同封装
    ...
    public List<RouterVo> findUserMenuListByUserId(Long userId) {
        ...
        if ((userId.longValue() == 1)) {
            ...
            wrapper.orderByAsc(SysMenu::getSortValue);//排序
        ...
        for (SysMenu menu : menus){
            ...
            if(menu.getType().intValue() ==1){
                ...
                if(!CollectionUtils.isEmpty(children)){
                    ...
                    //递归
                    router.setChildren(this.buildRouter(children));
                }
            }
            routers.add(router);
        }
    ...
    public List<String> findUserPermsByUserId(Long userId) {
        ...
        //3.从查询出来的数据里面，获取可以操作按钮值的list集合，最终进行返回
